...
RUN wget $GREENGRASS_RELEASE_URI
RUN chmod +x /greengrass-entrypoint.sh 
RUN mkdir -p /opt/greengrassv2 $GGC_ROOT_PATH && unzip $GREENGRASS_ZIP_FILE -d /opt/greengrassv2 && rm $GREENGRASS_ZIP_FILE

# Install Greengrass v2 dependencies
#RUN apt update -y && apt install -y python3.7 tar unzip wget sudo procps which && \
#    amazon-linux-extras enable python3.8 && apt install -y python3.8 java-11-amazon-corretto-headless && \
